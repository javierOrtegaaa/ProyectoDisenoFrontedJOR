...
export function PublicacionScreen({ route }) {
  ...
  const fetchPublicaciones = async () => {
    try {
      const url = 'http://192.168.1.11:8080/proyecto01/publicaciones';
  ...
  const fetchComentarios = async () => {
    try {
      const url = `http://192.168.1.11:8080/proyecto01/comentarios/${selectedPostId}`;
      const response = await fetch(url);
      if (!response.ok) {
  ...
  const handleLike = async (id) => {
    try {
      ...
      const url = `http://192.168.1.11:8080/proyecto01/publicaciones/put/${id}/${userId}`;
      const response = await fetch(url, {
        method: 'PUT',
  ...
  const handlePublishComment = async () => {
    ...
    try {
      const response = await fetch('http://192.168.1.11:8080/proyecto01/comentarios/put', {
        method: 'POST',
        headers: {
